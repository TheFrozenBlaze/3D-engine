...
public:
    ...
    void AABB() {
        ...
        for(size_t i = 0; i < cs.fvi.size(); i++) {
            for(size_t j = 0; j < cs.fvi[i].vec.size(), j++) {
                temporaryx.emplace_back(cs.xcoords[cs.fvi[i].vec[j].v]);
                temporaryy.emplace_back(cs.xcoords[cs.fvi[i].vec[j].v]);
                temporaryz.emplace_back(cs.xcoords[cs.fvi[i].vec[j].v]);
                if(*max_element(temporaryx.begin(), temporaryx.end())) {}
            }
        }
    };
};
